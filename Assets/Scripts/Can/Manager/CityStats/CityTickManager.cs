...
public class CityTickManager : MonoBehaviour
{
    ...
    {
        if (HappinessUiManager.Instance == null) return;

        float happiness = baseHappiness;

        // CO2 Last senkt Happiness
        if (Co2Manager.Instance != null)
            happiness -= Co2Manager.Instance.co2Value * co2Penalty;

        // Erneuerbare Energie erhöht Happiness
        if (renewable_energy.Instance != null)
        {
            int builtCount = 0;
            renewable_energy.Instance.Investments.ForEach(inv =>
            {
        ...
        HappinessUiManager.Instance.happinessValue = Mathf.Clamp(happiness, 0f, 100f);
    }
